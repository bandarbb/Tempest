...
For this, we'll be using Brim and look for the malicious domain that we found earlier.

I used this filter to get the overview of the conversation that happened between the malicious domains and the host but you can use a more specific filter.
...
The room provided us the filter so we will be using that.

![image](https://github.com/user-attachments/assets/b5d8fd50-94be-4342-9432-5fb46d47f128)


For now, let's decode everything.

![image](https://github.com/user-attachments/assets/5a67afbb-d5a3-4cc5-945b-e69bc1800291)


We would need to automate this if the log is larger, but we don't need to do that this time.
...
After googling each listening port, I found this.
...
Going back to Brim and removing the port filter
